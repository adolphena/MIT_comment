...
class RobotRunner : public PeriodicTask {
 public:
  ...
  RobotRunner(RobotController* , PeriodicTaskManager*, float, std::string);   /* 构造函数，第一个参数将将控制器类指针传进，
                                                                              第二个参数为SimulationBridge创建的PeriodicTaskManager类的指针；
                                                                              估计第四个参数为该周期任务的名字；
                                                                               */

  using PeriodicTask::PeriodicTask;
  void init() override;                                                       /* override关键字：提示重写父类函数，若未重写，则编译器报错 */
  void run() override;
